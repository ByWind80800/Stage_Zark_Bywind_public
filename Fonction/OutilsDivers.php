...
function TranfertPdf()
{
    $prenom_nomOrigine = $_FILES['PDF']['name'];
    $elementsChemin = pathinfo($prenom_nomOrigine);
    $extensionFichier = $elementsChemin['extension'];
    ...
    else
    {
        ...
        if (move_uploaded_file($_FILES['PDF']["tmp_name"],$repertoireDestination.$prenom_nomDestination))
        {
            echo "<p style= 'background-color: black; color:white'  >Le fichier temporaire ".$_FILES['PDF']["tmp_name"].
                " a été déplacé vers".$repertoireDestination.$prenom_nomDestination."</p>";
            return $prenom_nomDestination;
        }
        // else
